...
describe("ProfileCard Component", () => {
  ...
  beforeEach(async () => {
    ...
    element = await fixture<ProfileCard>(
      html`<profile-card
        .themePhoto=${mockThemePhoto}
        .bioText=${mockBioText}
      ></profile-card>`,
  ...
  it("should be defined as a custom element", () => {
    expect(element).to.be.instanceOf(ProfileCard);
  });
  ...
  it("should allow themePhoto property to be overridden", async () => {
    ...
    const customElement = await fixture<ProfileCard>(
      html`<profile-card .themePhoto=${customThemePhoto}></profile-card>`,
    );
